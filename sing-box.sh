...
# =========================
# SB-cfy-Pre: 老王sing-box四合一安装脚本 + Cloudflare优选
...
#!/usr/bin/env bash
bash <(curl -Ls https://raw.githubusercontent.com/Pretic/SB-cfy-Pre/main/sing-box.sh) $1
EOF
...
# SB-cfy Cloudflare优选节点生成器（内嵌 Pre-cfy 核心逻辑）
sb_cfy_check_deps() {
    local missing=0 cmd
    for cmd in jq curl base64 grep sed mktemp shuf awk paste; do
...
sb_cfy_get_vless_ps() {
    ...
    sb_cfy_url_decode "$ps"
}

sb_cfy_extract_vless_port() {
    local url="$1" rest endpoint
    rest="${url#*@}"
    endpoint="${rest%%\?*}"
    endpoint="${endpoint%%#*}"
...
sb_cfy_normalize_edge() {
    local edge="$1" fallback_port="$2"
    SB_CFY_EDGE_HOST="$edge"
    SB_CFY_EDGE_PORT="$fallback_port"

    if [[ "$edge" =~ ^\[([^]]+)\]:([0-9]+)$ ]]; then
        SB_CFY_EDGE_HOST="${BASH_REMATCH[1]}"
        SB_CFY_EDGE_PORT="${BASH_REMATCH[2]}"
    elif [[ "$edge" =~ ^([^:]+):([0-9]+)$ ]]; then
        SB_CFY_EDGE_HOST="${BASH_REMATCH[1]}"
        SB_CFY_EDGE_PORT="${BASH_REMATCH[2]}"
    elif [[ "$edge" =~ ^\[([^]]+)\]$ ]]; then
...
sb_cfy_update_vless_url() {
    local original_url="$1" new_add="$2" new_ps="$3"
    local port endpoint prefix rest suffix encoded_ps

    port=$(sb_cfy_extract_vless_port "$original_url")
    sb_cfy_normalize_edge "$new_add" "$port"
    endpoint=$(sb_cfy_format_host_port "$SB_CFY_EDGE_HOST" "$SB_CFY_EDGE_PORT")
    prefix="${original_url%%@*}@"
    rest="${original_url#*@}"
    suffix=""
    if [[ "$rest" == *\?* ]]; then
    ...
    encoded_ps=$(sb_cfy_url_encode "$new_ps")

    echo "${prefix}${endpoint}${suffix}#${encoded_ps}"
}

sb_cfy_update_vmess_url() {
    local original_json="$1" new_add="$2" new_ps="$3"
    local original_port modified_json new_base64

    original_port=$(echo "$original_json" | jq -r '.port // 443')
    sb_cfy_normalize_edge "$new_add" "$original_port"
    modified_json=$(echo "$original_json" | jq -c \
        --arg new_add "$SB_CFY_EDGE_HOST" \
        --arg new_port "$SB_CFY_EDGE_PORT" \
        --arg new_ps "$new_ps" \
        '.add = $new_add | .port = $new_port | .ps = $new_ps | del(.allowInsecure)')
    new_base64=$(printf '%s' "$modified_json" | sb_cfy_base64_one_line)
    echo "vmess://${new_base64}"
}

sb_cfy_cidr_to_usable_ip() {
...
sb_cfy_select_templates() {
    local url decoded_json ps path
    sb_cfy_valid_urls=()
    sb_cfy_valid_types=()
    sb_cfy_valid_ps_names=()
    ...
    while IFS= read -r url; do
        ...
        if [[ "$url" == vless://* ]]; then
            ...
            ps=$(sb_cfy_get_vless_ps "$url")
            sb_cfy_valid_urls+=("$url")
            sb_cfy_valid_types+=("vless")
            sb_cfy_valid_ps_names+=("$ps")
        elif [[ "$url" == vmess://* ]]; then
            decoded_json=$(printf '%s' "${url#"vmess://"}" | base64 -d 2>/dev/null)
            [ -n "$decoded_json" ] || continue
            path=$(echo "$decoded_json" | jq -r '.path // empty' 2>/dev/null)
            [[ "$path" == "/vmess-argo" || "$path" == "/vless-argo" ]] || continue
            ps=$(echo "$decoded_json" | jq -r '.ps // "vmess-ws-tls-argo"' 2>/dev/null)
            sb_cfy_valid_urls+=("$url")
            sb_cfy_valid_types+=("vmess")
            sb_cfy_valid_ps_names+=("$ps")
        fi
    done < "$client_dir"
}
...
sb_cfy_get_wetest_edges() {
    ...
    yellow "正在合并获取所有优选 IP (IPv4 & IPv6)..."
    for source in "${url_v4}|IPv4" "${url_v6}|IPv6"; do
        url="${source%%|*}"
        type_desc="${source#*|}"
        yellow "  -> 正在获取 ${type_desc} 列表..."
        html=$(curl -fsSL --connect-timeout 10 --max-time 30 "$url" 2>/dev/null || true)
        [ -n "$html" ] || { yellow "  -> 获取 ${type_desc} 列表失败"; continue; }
        table_rows=$(printf '%s' "$html" | tr -d '\n\r' | sed 's/<tr>/\n&/g' | grep '^<tr>' || true)
        ips=$(printf '%s' "$table_rows" | sed -n 's/.*data-label="优选地址">\([^<]*\)<.*/\1/p')
        isps=$(printf '%s' "$table_rows" | sed -n 's/.*data-label="线路名称">\([^<]*\)<.*/\1/p')
        paste -d' ' <(printf '%s\n' "$ips") <(printf '%s\n' "$isps") >> "$paired_data_file"
    done

    if ! [ -s "$paired_data_file" ]; then
        rm -f "$paired_data_file"
        red "无法从任何来源解析出优选 IP 地址。"
        return 1
    fi
    ...
    green "成功合并获取 ${#sb_cfy_ip_list[@]} 个优选 IP 地址，列表已随机打乱。"
}

sb_cfy_emit_url() {
    local url="$1"
    purple "$url"
    [ -n "${SB_CFY_OUTPUT:-}" ] && printf '%s\n' "$url" >> "$SB_CFY_OUTPUT"
}
...
sb_cfy_main() {
    ...
    green "\n=== Cloudflare优选节点生成器 ==="
    if [ "${#sb_cfy_valid_urls[@]}" -gt 0 ]; then
        if [ "${#sb_cfy_valid_urls[@]}" -eq 1 ]; then
            selected_index=0
            yellow "检测到一个可用模板，已自动选择: ${sb_cfy_valid_ps_names[0]}"
        else
            yellow "请选择一个节点作为模板:"
            for i in "${!sb_cfy_valid_ps_names[@]}"; do
                printf "%3d) [%s] %s\n" "$((i + 1))" "${sb_cfy_valid_types[$i]}" "${sb_cfy_valid_ps_names[$i]}"
            done
            while true; do
                reading "请输入选项编号 (1-${#sb_cfy_valid_urls[@]}): " choice
                if [[ "$choice" =~ ^[0-9]+$ ]] && [ "$choice" -ge 1 ] && [ "$choice" -le "${#sb_cfy_valid_urls[@]}" ]; then
        ...
        selected_url="${sb_cfy_valid_urls[$selected_index]}"
        selected_type="${sb_cfy_valid_types[$selected_index]}"
    else
        yellow "在 ${client_dir} 中未找到 vless-ws-tls-argo/vmess-argo 模板。"
        while true; do
            reading "请手动粘贴一个 vless:// 或 vmess:// 链接作为模板: " selected_url
            if [[ "$selected_url" == vless://* ]]; then
                selected_type="vless"
                break
            elif [[ "$selected_url" == vmess://* ]]; then
                if printf '%s' "${selected_url#"vmess://"}" | base64 -d >/dev/null 2>&1; then
                    selected_type="vmess"
                    break
                fi
            fi
            red "格式错误，必须是有效的 vless:// 或 vmess:// 链接。"
        done
    fi

    if [ "$selected_type" = "vless" ]; then
        original_ps=$(sb_cfy_get_vless_ps "$selected_url")
    else
        base64_part="${selected_url#"vmess://"}"
        original_json=$(printf '%s' "$base64_part" | base64 -d 2>/dev/null)
        original_ps=$(echo "$original_json" | jq -r '.ps // "vmess-ws-tls-argo"')
    fi
    green "已选择: ${original_ps}"
    ...
    if [ "$ip_source_type" = "wetest" ] || [ "$ip_source_type" = "import" ]; then
        for ((i=0; i<num_to_generate; i++)); do
            ...
            if [ "$selected_type" = "vless" ]; then
                ...
                generated=$(sb_cfy_update_vmess_url "$original_json" "$current_ip" "$new_ps")
            fi
            sb_cfy_emit_url "$generated"
        done
    else
        for ((i=0; i<num_to_generate; i++)); do
            random_ip_range="${cf_ranges[$((RANDOM % ${#cf_ranges[@]}))]}"
            ip_from_range=$(sb_cfy_cidr_to_usable_ip "$random_ip_range")
            new_ps="${original_ps}-CF$((i + 1))"
            if [ "$selected_type" = "vless" ]; then
                generated=$(sb_cfy_update_vless_url "$selected_url" "$ip_from_range" "$new_ps")
            else
                generated=$(sb_cfy_update_vmess_url "$original_json" "$ip_from_range" "$new_ps")
            fi
            sb_cfy_emit_url "$generated"
        done
    fi
    green "共生成 ${num_to_generate} 个链接。"
}

append_unique_generated_urls() {
    local source_file="$1" backup_file tmp_append appended=0 line

    [ -s "$source_file" ] || { yellow "没有可追加的优选节点。"; return 1; }
    mkdir -p "$work_dir"
    touch "$client_dir" || { red "无法写入 ${client_dir}"; return 1; }

    backup_file="${client_dir}.bak.$(date +%Y%m%d%H%M%S)"
    cp "$client_dir" "$backup_file" || { red "备份 ${client_dir} 失败。"; return 1; }

    tmp_append=$(mktemp)
    while IFS= read -r line; do
        [ -z "$line" ] && continue
        if ! grep -Fxq "$line" "$client_dir"; then
            printf '%s\n' "$line" >> "$tmp_append"
            appended=$((appended + 1))
        fi
    done < "$source_file"
    ...
    if [ "$appended" -gt 0 ]; then
        [ -s "$client_dir" ] && printf '\n' >> "$client_dir"
        cat "$tmp_append" >> "$client_dir"
        base64 -w0 "$client_dir" > "${work_dir}/sub.txt" 2>/dev/null || base64 "$client_dir" | tr -d '\n\r' > "${work_dir}/sub.txt"
        chmod 644 "${work_dir}/sub.txt"
        green "已追加 ${appended} 个优选节点并刷新订阅。备份: ${backup_file}"
    else
        yellow "没有新增节点，当前订阅中已存在这些链接。备份: ${backup_file}"
    fi
...
run_cfy_generator() {
    local cfy_output="${work_dir}/cfy.txt" append_choice
    ...
    export SB_CFY_OUTPUT="$cfy_output"
    : > "$cfy_output" || {
        red "无法写入 ${cfy_output}"
        return 1
    }

    sb_cfy_main || return 1

    if [ -s "$cfy_output" ]; then
        green "\n生成结果已保存到: ${cfy_output}"
        if [ -f "$client_dir" ]; then
            reading "是否追加生成的优选节点到当前订阅并刷新 sub.txt? (y/N): " append_choice
            if [[ "$append_choice" == "y" || "$append_choice" == "Y" ]]; then
                append_unique_generated_urls "$cfy_output"
            else
                yellow "未追加。可先检查 ${cfy_output} 后再手动处理。"
            fi
        else
            yellow "当前未发现 ${client_dir}，已仅保存生成结果。"
        fi
    else
        yellow "没有生成任何节点链接。"
    fi
...
menu() {
    ...
    green "Github地址: ${purple}https://github.com/Pretic/SB-cfy-Pre${re}\n"
        ...
        while true; do
            ...
            case "${choice}" in
                ...
                10)
                    ...
                    need_pause=false
                    ;;
                11)  run_cfy_generator;   need_pause=true ;;
                0)  exit 0 ;;       
                *)
                    red "无效的选项，请输入 0-11"
                    need_pause=true

...
sing-box 多协议代理脚本 + 内嵌 Cloudflare 优选节点生成器。它支持 VLESS-reality-version、VLESS-WS-TLS(Argo)、Hysteria2、Tuic，并可在菜单中直接生成和追加 Cloudflare 优选入口节点。
...
本仓库是新的 SB-cfy-Pre 项目，基于 `Sing-box-Pre` 最小合并 `Pre-cfy` 核心功能，不代表上游项目。二改重点集中在 VPS 一键脚本 `sing-box.sh`：
...
```
PORT=你的端口 bash <(curl -Ls https://raw.githubusercontent.com/Pretic/SB-cfy-Pre/main/sing-box.sh)
```
